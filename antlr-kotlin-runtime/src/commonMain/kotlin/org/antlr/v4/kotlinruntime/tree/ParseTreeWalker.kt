...
public open class ParseTreeWalker {
   ...
   * @param t The parse tree to be walked on
   */
  public open suspend fun walk(listener: SuspendParseTreeListener, t: ParseTree) {
    if (t is ErrorNode) {
      listener.visitErrorNode(t)
      return
    } else if (t is TerminalNode) {
   ...
   * @param r The grammar rule containing the rule context
   */
  protected open suspend fun enterRule(listener: SuspendParseTreeListener, r: RuleNode) {
    val ctx = r.ruleContext as ParserRuleContext
    listener.enterEveryRule(ctx)
    ctx.enterRule(listener)
  }
   ...
   * @param r The grammar rule containing the rule context
   */
  protected open suspend fun exitRule(listener: SuspendParseTreeListener, r: RuleNode) {
    val ctx = r.ruleContext as ParserRuleContext
    ctx.exitRule(listener)
    listener.exitEveryRule(ctx)
  }
